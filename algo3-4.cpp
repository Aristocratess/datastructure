...
struct SElemType//栈的元素类型
  {
	...
	int di;//从此通块走向下一通道的“方向”（0~3表示东~北） 
   } ;
#include"c3-1.h"//采取顺序栈存储结构
...
{//使迷宫m的b点值变为-1(标记为经试探由此不能到达终点的路径）
  m[b.x][b.y]=-1; 
}
Status MazePath(PosType start,PosType end)
{//若迷宫m中存在从入口start到出口end的通道，则求得一条存放在栈中（从栈底到栈顶），并返回TRUE，否则返回ERROR
  ...
  do{
	...
	else//当前位置不能通过
	{
		...
		{Pop(S,e);//退栈到前一位置
		curstep--;//足迹减1
		while(e.di==3&&!StackEmpty(S))//当前一位置留下由其不能到达终点的标记(-1)
		Pop(S,e);
		curstep--;//足迹再减1 
		}
