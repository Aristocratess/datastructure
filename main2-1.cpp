...
#include"func2-2.cpp"//包括equal(),comp(),print()和print2()函数 
Status sq(ElemType c1,ElemType c2)
{//数据元素判断函数（平方关系），LocateElem()调用的函数
   if(c1==c2*c2)
     return TRUE;
   else
     return FALSE;	 
...
int main()
{
	...
	ListTraverse(L,print1);//依次对表L中的元素调用printf()函数（输出元素的值）
	i=ListEmpty(L);//检测表L是否空
	printf("L.length=%d(改变),L.listsize=%d,",L.length,L.listsize);
	...
	for(j=10;j<=11;j++)
	{
	 ...
	 for(j=ListLength(L)-1;j<=ListLength(L);j++)//最后2个数据
	 {
	  ...
	  for(j=k+1;j>=k;j--)
	  {
	   ...
	   ListTraverse(L,dbl);//依次对元素调用db1()，元素值乘2
	   printf("L的元素值加倍后，L=");
